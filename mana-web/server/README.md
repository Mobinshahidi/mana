# Mana Web - SMS Parser Tool

A web-based tool for testing and reporting SMS parsing issues for the Mana AI app.
...
# Run the container
docker run -p 8080:8080 -e DATABASE_PASSWORD=your_password mana-web
```

### Using systemd (Linux)

1. Create a service file at `/etc/systemd/system/mana-web.service`:
   ```ini
   [Unit]
   Description=Mana Web SMS Parser
   After=network.target

   [Service]
   Type=simple
   User=www-data
   WorkingDirectory=/opt/mana-web
   Environment="DATABASE_PASSWORD=your_password"
   ExecStart=/usr/bin/java -jar /opt/mana-web/server-all.jar
   Restart=always
...
2. Start the service:
   ```bash
   sudo systemctl enable mana-web
   sudo systemctl start mana-web
   ```
