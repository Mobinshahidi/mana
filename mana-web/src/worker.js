...
export class ManaContainer extends Container {
  ...
  onStart() {
    console.log("Mana container successfully started");
  }

  onStop() {
    console.log("Mana container successfully shut down");
  }

  onError(error) {
    console.log("Mana container error:", error);
  }
}
...
app.all("*", async (c) => {
  ...
  const container = c.env.MANA_CONTAINER.getByName("main");
