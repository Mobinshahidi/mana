...
import com.mana.data.parser.CustomSmsParser
...
import com.mana.data.mapper.toEntity
import com.mana.data.mapper.toEntityType
import com.mana.data.preferences.UserPreferencesRepository
import com.mana.data.repository.AccountBalanceRepository
import com.mana.data.repository.CardRepository
import com.mana.data.repository.LlmRepository
import com.mana.data.repository.MerchantMappingRepository
import com.mana.data.repository.SubscriptionRepository
import com.mana.data.repository.TransactionRepository
import com.mana.data.repository.UnrecognizedSmsRepository
import com.mana.domain.repository.RuleRepository
import com.mana.domain.service.RuleEngine
...
private suspend fun processSubscriptionNotifications(
    parser: com.mana.parser.core.bank.BankParser,
    sms: SmsMessage,
    smsDateTime: LocalDateTime,
    isRecentMessage: Boolean
): SubscriptionResult {
    return SubscriptionResult(false, 0)
}
...
private suspend fun saveParsedTransaction(
    ...
    return try {
        ...
        val inferenceResult: SmartParser.BalanceInferenceResult?
        val effectiveTransaction: ParsedTransaction

        if (
            parsedTransaction.type == ParserTransactionType.TRANSFER &&
            ...
            if (existingBalance != null) {
                ...
                if (result.type != null) {
                    if (result.gapDetected) {
                        Log.d(TAG, "Inferred TRANSFER as ${result.type} (gap=${result.gapAmount}, small SMS missing)")
                    } else {
                        Log.d(TAG, "Inferred TRANSFER type from balance: ${result.type}")
                    }
                    inferenceResult = result
                    effectiveTransaction = parsedTransaction.copy(type = result.type!!)
                } else {
                    if (result.isBankPause) {
                        Log.w(TAG, "Bank pause: balance delta ${result.gapAmount} doesn't match transaction, keeping as TRANSFER")
                    } else {
                        Log.d(TAG, "Could not infer TRANSFER type from balance (SMS may be missing)")
                    }
                    inferenceResult = result
                    effectiveTransaction = parsedTransaction
                }
            } else {
...
private suspend fun processBalanceUpdate(
    parsedTransaction: ParsedTransaction,
    entity: com.mana.data.database.entity.TransactionEntity,
    rowId: Long,
    ...
    if (parsedTransaction.accountLast4 != null) {
        ...
        val targetAccountLast4 = if (isFromCard) {
            ...
            val result = when {
                card.cardType == com.mana.data.database.entity.CardType.CREDIT -> {
                    // Credit cards get balance entries
                    Log.d(TAG, "CREDIT card - will create balance entry")
                    parsedTransaction.accountLast4
                }

                card.cardType == com.mana.data.database.entity.CardType.DEBIT && card.accountLast4 != null -> {
        ...
        if (targetAccountLast4 != null) {
            val isCreditCard = (parsedTransaction.type.toEntityType() == TransactionType.CREDIT) ||
                    ...
                    } == com.mana.data.database.entity.CardType.CREDIT
...
private suspend fun createSyntheticGapTransaction(
    ...
    parsedTransaction: ParsedTransaction,
    parentEntity: com.mana.data.database.entity.TransactionEntity
) {
    ...
    val gapTransaction = com.mana.data.database.entity.TransactionEntity(
        amount = gapAmount,
