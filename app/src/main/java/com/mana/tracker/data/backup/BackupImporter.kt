package com.mana.tracker.data.backup
...
import com.mana.tracker.data.database.ManaDatabase
import com.mana.tracker.data.database.entity.*
...
class BackupImporter @Inject constructor(
    ...
    private val database: ManaDatabase,
     ...
     */
    private suspend fun readBackupFile(uri: Uri): ManaBackup {
        return withContext(Dispatchers.IO) {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                val reader = BufferedReader(InputStreamReader(inputStream))
                val content = reader.readText()
                gson.fromJson(content, ManaBackup::class.java)
     ...
     */
    private fun isCompatibleVersion(backup: ManaBackup): Boolean {
        // For now, accept all v1.x backups
        return backup.format.startsWith("Mana Backup v1")
    }
    
    /**
     * Replace all existing data with backup data
     */
    private suspend fun replaceAllData(backup: ManaBackup): ImportResult {
     ...
     */
    private suspend fun mergeData(backup: ManaBackup): ImportResult {
        var importedTransactions = 0
        var importedCategories = 0
