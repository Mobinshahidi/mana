package com.mana.tracker.worker
...
import com.mana.tracker.data.preferences.UserPreferencesRepository
import com.mana.tracker.data.repository.AccountBalanceRepository
import com.mana.tracker.data.repository.CardRepository
import com.mana.tracker.data.repository.LlmRepository
import com.mana.tracker.data.repository.MerchantMappingRepository
import com.mana.tracker.data.repository.SubscriptionRepository
import com.mana.tracker.data.repository.TransactionRepository
import com.mana.tracker.data.repository.UnrecognizedSmsRepository
import com.mana.tracker.domain.repository.RuleRepository
import com.mana.tracker.domain.service.RuleEngine
import com.mana.tracker.utils.CurrencyFormatter
...
private suspend fun processBalanceUpdate(
    parsedTransaction: ParsedTransaction,
    entity: com.mana.tracker.data.database.entity.TransactionEntity,
    rowId: Long,
    ...
    if (parsedTransaction.accountLast4 != null) {
        ...
        val targetAccountLast4 = if (isFromCard) {
            ...
            val result = when {
                card.cardType == com.mana.tracker.data.database.entity.CardType.CREDIT -> {
                    // Credit cards get balance entries
                    Log.d(TAG, "CREDIT card - will create balance entry")
                    parsedTransaction.accountLast4
                }

                card.cardType == com.mana.tracker.data.database.entity.CardType.DEBIT && card.accountLast4 != null -> {
        ...
        if (targetAccountLast4 != null) {
            val isCreditCard = (parsedTransaction.type.toEntityType() == TransactionType.CREDIT) ||
                    ...
                    } == com.mana.tracker.data.database.entity.CardType.CREDIT
...
private suspend fun createSyntheticGapTransaction(
    ...
    parsedTransaction: ParsedTransaction,
    parentEntity: com.mana.tracker.data.database.entity.TransactionEntity
) {
    ...
    val gapTransaction = com.mana.tracker.data.database.entity.TransactionEntity(
        amount = gapAmount,
